 ...
 */
@Component
public class WhitelistProcess implements ContextHandler<SensitveEffectiveContext, SensitiveEffectRes> {
    ...
    public void handle(SensitveEffectiveContext context, SensitiveEffectRes nextDeal) {
        ...
        try {
            ...
            List<SensitiveWord> hitWords = getWhitelistSensitiveWord(context.getHitWords());
    ...
    private List<SensitiveWord> getWhitelistSensitiveWord(List<SensitiveWord> hitWords) {
        if (CollectionUtils.isEmpty(hitWords)) {
            return Lists.newArrayList();
        }
        List<SensitiveWord> whitelistSensitiveWord = Arrays.asList(SensitiveWord.builder().sensitive("张彦峰").build(),
                SensitiveWord.builder().sensitive("张靓颖").build());
        List<String> whitelistSensitiveWordInfo = whitelistSensitiveWord.stream().map(SensitiveWord::getSensitive).collect(Collectors.toList());
        return hitWords.stream().peek(sensitiveWord -> {
            if (whitelistSensitiveWordInfo.contains(sensitiveWord.getSensitive())) {
                sensitiveWord.setWhitened(true);
            }
