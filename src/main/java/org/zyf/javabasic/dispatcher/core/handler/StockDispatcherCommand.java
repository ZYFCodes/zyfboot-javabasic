 ...
 * @description: 机构股票内容分发
 * @author: zhangyanfeng
 * @create: 2024-04-21 20:09
 **/
@Service
public class StockDispatcherCommand extends DispatcherCommand implements InitializingBean {
    @Override
    protected List<DistributionContent> fetchDistributionContent(DispatcherInfo dispatcherInfo) {
        List<DistributionContent> distributions = dispatcherInfo.getDispatcherInfo().get(DistributionAnalyzeType.STOCK);
        //机构类型的基金可直接进行分发
        return distributions.stream().filter(DistributionContent::isValid)
                .filter(DistributionContent::isMechanism).collect(Collectors.toList());
    }
    ...
    protected void distributeContent(List<DistributionContent> contents, TargetUserCriteria targetUser, DistributionStrategy distributionStrategy, DispatchTiming dispatchTiming) {
        System.out.println("股票分发处理...........");
    }

    @Override
    public void afterPropertiesSet() throws Exception {
        DistributeExecutor.register(DistributionAnalyzeType.STOCK, this);
    }
}
